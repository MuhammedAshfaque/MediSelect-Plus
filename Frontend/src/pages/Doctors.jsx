    ...
    <div>
      ...
      <div className='flex flex-col sm:flex-row items-start gap-5 mt-5'>
        ...
        <div className={`flex flex-col gap-4 text-sm text-gray-600 ${showfilter ? 'flex' : 'hidden sm:flex'}`}>
          ...
          <p onClick={()=> speciality ==='Pediatricians'? navigate('/doctor') : navigate('/doctor/Pediatricians')} className={`w-[94vw] sm:w-auto pl-3 py-1.5 pr-16 border border-gray-300 rounded transition-all cursor-pointer ${speciality=== "Pediatricians" ? "bg-indigo-100 text-black" : ""}`}>Padiatricians</p>
        ...
        <div className='w-full grid md:grid-cols-5 grid-cols-2 gap-4 pt-5 gap-y-6 px-3 sm:px-0'>
          {
          FilterDoc.map((item, index) => (
                  <div onClick={()=>navigate(`/appoinment/${item._id}`)} className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-6px] transition-all duration-500' key={index}>
